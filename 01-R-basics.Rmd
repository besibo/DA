...
Regardez [cette vidéo DataCamp](https://campus.datacamp.com/courses/working-with-the-rstudio-ide-part-1/orientation?ex=5) pour découvrir les différents panneaux de l'application, en particulier la Console dans laquelle nous exécuterons très bientôt du code R.
...
* Le package `dplyr` pour les manipuler des tableaux de données dans le chapitre \@ref(wrangling)
...
Créez un nouveau script que vous nommerez `VotreNomDeFamille.R`. Vous prendrez soin d'ajouter autant de commentaires que nécessaire dans ce script afin de le structurer correctement.

...
Les variables `Sexe`, `Statut` et `Organe` devrait elles aussi être encodées sous forme de facteurs. Plutôt que de taper le code ci dessous (qui fonctionne très bien) :
...
Nous pouvons maintenant modifier l'ordre des niveaux de `Satut` :
...
Outre les fonction `fct_rev()`, `fct_recode()` et `fct_relevel()` abordées ici, on peut aussi noter : 
